...
def Q_1(cursor, conn, execution_time):
    ...
    query = """SELECT player_name, AVG(xg_score) AS average_xg_score
                ...
                ON public.shots.event_id = public.events.event_id
                INNER JOIN public.matches
                ON public.matches.match_id = public.events.match_id
...
def Q_2(cursor, conn, execution_time):
    ...
    query = """SELECT player_name, COUNT(shots.event_id) as num_shots
                ...
                INNER JOIN public.matches
                ON public.matches.match_id = public.events.match_id
				WHERE (competition_id = 11 AND season_id = 90)
				GROUP BY player_id
...
def Q_3(cursor, conn, execution_time):
    ...
    query = """SELECT player_name, COUNT(player_id) AS num_first_times
                FROM public.shots 
                INNER JOIN public.players 
                ON public.shots.initiating_player = public.players.player_id
                INNER JOIN public.events
                ...
                INNER JOIN public.matches
                ON public.matches.match_id = public.events.match_id
                WHERE (first_time = true) AND (competition_id = 11)
                GROUP BY player_id
                ORDER BY num_first_times DESC
    """
...
def Q_4(cursor, conn, execution_time):
    ...
    query = """SELECT team_name, COUNT(events.event_id) AS num_passes
                FROM public.passes
                INNER JOIN public.events
                ...
                INNER JOIN public.teams
                    ON public.teams.team_id = public.events.event_team_id
                WHERE (competition_id = 11 AND season_id = 90)
                GROUP BY team_name
...
def Q_6(cursor, conn, execution_time):
    ...
    query = """SELECT team_name, COUNT(shots.event_id) AS num_shots
                FROM public.shots
                INNER JOIN public.events
                    ON public.shots.event_id = public.events.event_id
                INNER JOIN public.matches
...
def Q_7(cursor, conn, execution_time):
    ...
    query = """ SELECT player_name, COUNT(passes.event_id) AS num_through_balls
                ...
                INNER JOIN public.players
                    ON passes.initiating_player = players.player_id
                WHERE (competition_id = 11 AND season_id = 90) AND (through_ball = true)
                GROUP BY player_name
...
def Q_8(cursor, conn, execution_time):
    ...
    query = """SELECT team_name, COUNT(passes.event_id) AS num_through_balls
                FROM public.passes
                INNER JOIN public.events
                    ON passes.event_id = events.event_id
                INNER JOIN public.matches
...
def Q_9(cursor, conn, execution_time):
    ...
    query = """SELECT player_name, COUNT(dribbles.event_id) AS num_completed_dribbles
                FROM public.dribbles
                INNER JOIN public.events
                    ON dribbles.event_id = events.event_id
                INNER JOIN public.matches
                    ON events.match_id = matches.match_id
                INNER JOIN public.players
                    ON dribbles.initiating_player = players.player_id
                WHERE (competition_id = 11) AND (completed = true)
                GROUP BY player_name
                ORDER BY num_completed_dribbles DESC
                """
...
def Q_10(cursor, conn, execution_time):
    ...
    query = """SELECT player_name, COUNT(dribbles_past.event_id) AS num_times_dribbled_past
                FROM public.dribbles_past
                INNER JOIN public.events
                    ON dribbles_past.event_id = events.event_id
                INNER JOIN public.matches
                    ON events.match_id = matches.match_id
                INNER JOIN public.players
                    ON dribbles_past.initiating_player = players.player_id
                WHERE competition_id = 11 AND season_id = 90
                GROUP BY player_name
                ORDER BY num_times_dribbled_past ASC
                """
